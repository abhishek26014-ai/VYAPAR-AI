...
function Workspace() {
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(async () => {
      try {
        ...
        setSavedMessage(requestError instanceof Error ? requestError.message : 'Unable to save business profile');
      }
    }, 400);
  ...
  const refreshData = async () => {
    try {
      ...
      setSavedMessage(requestError instanceof Error ? requestError.message : 'Unable to refresh data');
    }
  ...
  const submitRecord = async (e: FormEvent) => {
    ...
    try {
      ...
      setSavedMessage(requestError instanceof Error ? requestError.message : 'Unable to save financial record');
      return;
    }
    setForm(emptyForm);
    setEditingId(null);
  ...
  const deleteRecord = async (id: number) => {
    try {
      ...
      setSavedMessage(requestError instanceof Error ? requestError.message : 'Unable to delete record');
    }
    setTimeout(() => setSavedMessage(''), 1800);
  };
    ...
    <>
      ...
      <section className="chart-grid two-thirds">
        <div className="panel large-panel">
          ...
          {/* FIX: Added explicit height to chart-area here */}
          <div className="chart-area" style={{ height: '260px', width: '100%' }}><ResponsiveContainer width="100%" height="100%"><BarChart data={revenueTrend} margin={{top:10,right:10,left:-18,bottom:0}}><CartesianGrid stroke="#edf3ef" vertical={false}/><XAxis dataKey="month" axisLine={false} tickLine={false} tick={{fill:'#94a39a',fontSize:11}}/><YAxis axisLine={false} tickLine={false} tick={{fill:'#94a39a',fontSize:11}} tickFormatter={(v)=>`₹${v}k`}/><Tooltip formatter={(value)=>[currency(Number(value || 0)*1000),'Revenue']}/><Bar dataKey="value" radius={[7,7,2,2]} fill="#0d8b62"/></BarChart></ResponsiveContainer></div>
        </div>
        <div className="panel">
          <div className="panel-header"><div><h3>Cash flow</h3><p>Revenue minus operating expenses</p></div></div>
          {/* FIX: Added explicit height to compact chart-area here */}
          <div className="chart-area compact" style={{ height: '220px', width: '100%' }}><ResponsiveContainer width="100%" height="100%"><AreaChart data={cashFlowData} margin={{top:15,right:5,left:-18,bottom:0}}><defs><linearGradient id="cashGradient2" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#0d8b62" stopOpacity={0.22}/><stop offset="100%" stopColor="#0d8b62" stopOpacity={0}/></linearGradient></defs><CartesianGrid stroke="#edf3ef" vertical={false}/><XAxis dataKey="month" axisLine={false} tickLine={false} tick={{fill:'#94a39a',fontSize:10}}/><YAxis axisLine={false} tickLine={false} tick={{fill:'#94a39a',fontSize:9}} tickFormatter={(v)=>`${v}k`}/><Tooltip formatter={(value)=>[`₹${Number(value || 0)}k`,'Net cash']}/><Area type="monotone" dataKey="net" stroke="#0d8b62" strokeWidth={2.4} fill="url(#cashGradient2)"/></AreaChart></ResponsiveContainer></div>
    ...
    <>
      ...
      <section className="entry-layout">
        ...
        <div className="panel profile-panel">
          <div className="panel-header"><div><h3>Business profile</h3><p>Synced with your workspace</p></div></div>
          ...
          <div className="info-box"><ShieldAlert size={16}/><div><strong>Connected workspace</strong><p>Business details and financial records are saved to your FastAPI backend.</p></div></div>
  ...
  const renderReports = () => (
    <section className="page-grid-single"><div className="panel"><div className="panel-header"><div><h3>Reports</h3><p>Quick snapshot based on your saved data.</p></div><button className="primary-btn" onClick={()=>window.print()}><Upload size={15}/> Print / Save PDF</button></div><div className="report-grid"><div><span>Business</span><strong>{business.name}</strong></div><div><span>Health</span><strong>{health}/100</strong></div><div><span>Risk</span><strong>{risk}</strong></div><div><span>Revenue</span><strong>{currency(current?.revenue || 0)}</strong></div><div><span>Profit</span><strong>{currency(profit)}</strong></div><div><span>Cash</span><strong>{currency(current?.cashBalance || 0)}</strong></div><div><span>Receivables</span><strong>{currency(current?.receivables || 0)}</strong></div><div><span>Debt</span><strong>{currency(current?.loanOutstanding || 0)}</strong></div></div></div><div className="panel"><div className="panel-header"><div><h3>Notes</h3><p>Report details</p></div></div><p className="report-note">This report reflects the financial records currently saved in your connected workspace.</p></div></section>
  );
  ...
  return (
    <div className="app-shell">
      <aside className={`sidebar ${sidebarOpen ? 'expanded' : 'collapsed'} ${mobileOpen ? 'mobile-visible' : ''}`}>
        ...
        <div className="sidebar-bottom"><button className="nav-item" onClick={async () => { try { await authApi.logout(); } finally { localStorage.removeItem('vyaparai-token'); window.location.reload(); } }}><Settings size={18}/>{sidebarOpen&&<span>Logout</span>}</button><div className="trust-chip"><ShieldAlert size={15}/>{sidebarOpen&&<span>Protected workspace</span>}</div></div>
      </aside>
...
function AuthScreen() {
  ...
  return <main className="auth-shell">
    <section className="auth-card">
      <div className="brand-row"><div className="brand-mark"><span>V</span></div><div><div className="brand-name">Vyapar<span>AI</span></div><div className="brand-sub">Financial Intelligence</div></div></div>
      <div className="eyebrow green">SECURE WORKSPACE</div>
      <h1>{mode === 'login' ? 'Welcome back.' : 'Create your workspace.'}</h1>
      <p>{mode === 'login' ? 'Sign in to continue to your financial command center.' : 'Start with your business details. You can complete the profile later.'}</p>
      <form onSubmit={submit} className="auth-form">
        {mode === 'register' && <><label>Full name<input required value={form.full_name} onChange={e => setForm({ ...form, full_name: e.target.value })} /></label><label>Business name<input required value={form.business_name} onChange={e => setForm({ ...form, business_name: e.target.value })} /></label><label>Phone<input value={form.phone} onChange={e => setForm({ ...form, phone: e.target.value })} /></label><label>Business type<select value={form.business_type} onChange={e => setForm({ ...form, business_type: e.target.value })}><option>Retail</option><option>Wholesale</option><option>Manufacturing</option><option>Services</option></select></label></>}
        ...
        <button className="primary-btn" disabled={busy}>{busy ? 'Connecting...' : mode === 'login' ? 'Login' : 'Create account'}</button>
      </form>
      <button className="auth-switch" onClick={() => { setMode(mode === 'login' ? 'register' : 'login'); setError(''); }}>{mode === 'login' ? 'Need an account? Register' : 'Already have an account? Login'}</button>
    </section>
  </main>;
}
